...
# récupère le 2ème élément du couple (a,b) de la liste quand x = a
def getElem(liste, a) :
    for k in range(0,len(liste)-1) :
...
# paramètres : k = taille de l'ensemble des nombres premiers, borne = borne de l'algorithme, n = nombre à factoriser, div = 
l'ensemble des produits de n déjà trouvés   
def remplir_ens_r(k, borne, n, div) :
    m = 0  
    r = []
    while m < k+1 :
        x = randint(1,floor(sqrt(n)))
        y = x**2 % n
...
def calcule_div_nb(nb,k,matrice, matriceMod2, r, prems) :
    e = matriceMod2.right_kernel() # vecteur non nul noyau de m         
...
def dixon (n) :
    ...
    for i in range(0,len(prems)) : 
        while nb%prems[i] == 0 : nb = nb/prems[i]
    ...
    while prod_div != n :        
        ...
        #diviseur = calcule_div_nb(nb,k,matrice,r, prems)
...
def test(nb) :
    ...
    div = dixon(nb)
    print "nombre à factoriser : ", nb
    print "les facteurs : ", div
    print "temps d'exécution (en secondes) : ", cputime(t)
    #if prod(div) == nb : return "ok"
    #else : return "erreur factorisation"
